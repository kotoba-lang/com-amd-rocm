(ns amd_rocm.main
  "Kotodama WASM entrypoint for the Amd_rocm clean-room actor (L4) — Clojure port.
  ...
  facts (`amd_rocm.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "amd_rocm")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"amd_rocm.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "amd_rocm-compat" :tier tier :entities entities} 200])
...
(defn start! [] :amd_rocm-compat/ready)
